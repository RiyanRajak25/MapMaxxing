/**
 * A rotating globe for the two waits in this tool.
 *
 * Both are Earth Engine waits and both are long enough to look like a hang: the
 * analysis request runs about a minute, and every result tile is rendered on
 * demand the first time it is requested. One loader for both, so the interface
 * says "the server is working" the same way in each case.
 *
 * Everything inside .earth-loader is sized in `em`, so the whole thing scales
 * from a single font-size. `size` is the globe's diameter in pixels; the 7.5
 * divisor is the width in em declared in App.css and the two must stay in step.
 *
 * Decorative: the surrounding overlay carries the live region and the text, so
 * this is hidden from assistive tech rather than announced as four blobs.
 */
const EM_PER_DIAMETER = 7.5

// The continents. Four irregular blobs at staggered offsets and delays - they
// read as landmasses drifting past rather than as one shape going round.
const CONTINENTS = [
...
export default function EarthLoader({ size = 120 }) {
  return (
    <div
      className="earth-loader"
      style={{ fontSize: `${Number((size / EM_PER_DIAMETER).toFixed(3))}px` }}
      aria-hidden="true"
    >
      {CONTINENTS.map((d) => (
        <svg key={d} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200">
          <path transform="translate(100 100)" d={d} />
        </svg>
      ))}
    </div>
  )
